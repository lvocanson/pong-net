...
void ConnectionState::InitLobbyButton(const sf::Vector2f& pos)
{
	...
	m_ShowLobbyBtn.SetButtonText(btnText, m_ClientApp.GetFont(), TEXT_COLOR, textSize / 2.f);
}
...
void ConnectionState::InitQuickGameButton(const sf::Vector2f& pos)
{
	...
	m_QuickGameBtn.SetButtonText(btnText, m_ClientApp.GetFont(), TEXT_COLOR, textSize / 2.f);
}
